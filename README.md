...
- A generated Telethon `StringSession` string
...
### Step 2 — Generate a Telethon StringSession
...
Copy the printed string — this is your `TELEGRAM_SESSION` secret.
...
| `TELEGRAM_SESSION` | The StringSession string from Step 2 |

### Step 4 — Configure Channels

Edit the `CHANNELS` list in **both** files to match the Telegram channels you want to mirror:

**`Fetch-feeds.sh`** — all channels (rotation pool):
```bash
CHANNELS=("channel1" "channel2" "channel3" ...)
```

**`bridge.py`** — channels for MTProto media pre-fetching:
```python
CHANNELS = ["channel1", "channel2", "channel3", ...]
```
...
The workflow runs automatically every 30 minutes. You can also trigger it manually via **Actions → Smart Telegram Content-Reader → Run workflow**.
