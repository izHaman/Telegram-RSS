...
### 🎬 بنر رسانه‌ای دوزبانه
- برای هر پست **ویدیو، صدا، یا GIF**، یک بنر HTML استایل‌دار درون `<description>` تزریق می‌شود.
- در هر خواننده RSS مبتنی بر WebView رندر می‌شود (Feeder، Reeder، NetNewsWire و غیره).
- **دوزبانه**: انگلیسی + فارسی (`پخش رسانه | Open Media`).
- CSS inline خالص — بدون JavaScript، بدون درخواست خارجی، طراحی با پس‌زمینه تیره‌گرادیانت.
- تمام بنر قابل لمس است — هیچ target کوچکی وجود ندارد.
...
- یک ریپازیتوری GitHub (عمومی یا خصوصی)
- یک حساب تلگرام با اعتبارنامه API از [my.telegram.org](https://my.telegram.org)
- یک رشته Telethon `StringSession` تولید شده
...
### مرحله ۲ — تولید StringSession تلثون
...
api_hash = "your_hash"  # از my.telegram.org

with TelegramClient(StringSession(), api_id, api_hash) as client:
    print(client.session.save())
```

رشته چاپ‌شده را کپی کنید — این همان secret `TELEGRAM_SESSION` شما است.

### مرحله ۳ — پیکربندی GitHub Secrets

به **Settings → Secrets and variables → Actions → New repository secret** بروید و موارد زیر را اضافه کنید:

| نام Secret | مقدار |
|---|---|
| `TELEGRAM_API_ID` | شناسه API عددی از my.telegram.org |
| `TELEGRAM_API_HASH` | API hash از my.telegram.org |
| `TELEGRAM_SESSION` | رشته StringSession از مرحله ۲ |

### مرحله ۴ — پیکربندی کانال‌ها

لیست `CHANNELS` را در **هر دو** فایل زیر ویرایش کنید تا با کانال‌های تلگرامی که می‌خواهید mirror کنید مطابقت داشته باشد:

**`Fetch-feeds.sh`** — همه کانال‌ها (pool چرخش):
```bash
CHANNELS=("channel1" "channel2" "channel3" ...)
```

**`bridge.py`** — کانال‌ها برای پیش‌دانلود رسانه از MTProto:
```python
CHANNELS = ["channel1", "channel2", "channel3", ...]
```

### مرحله ۵ — فعال‌سازی دسترسی نوشتن GitHub Actions

به **Settings → Actions → General → Workflow permissions** بروید و **Read and write permissions** را انتخاب کنید.

### مرحله ۶ — اجرا

Workflow به‌صورت خودکار هر ۳۰ دقیقه اجرا می‌شود. همچنین می‌توانید آن را به‌صورت دستی از طریق **Actions → Smart Telegram Content-Reader → Run workflow** راه‌اندازی کنید.
